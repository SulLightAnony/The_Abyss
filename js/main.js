...
function initGameEngine() {
    document.getElementById('item-max').innerText = LEVEL_DATA[State.currentLevel].State.items; document.getElementById('item-count').innerText = "0";
    ...
    State.camera = new THREE.PerspectiveCamera(70, window.innerWidth / window.innerHeight, 0.1, 800); State.camera.position.set(0, State.player.height, 0); State.scene.add(State.camera); lookState.yaw = 0; lookState.pitch = 0;
    buildCurrentLevel(); setupLighting(); if(droneOsc) droneOsc.frequency.value = (State.currentLevel===0 ? 50 : (State.currentLevel===1 ? 80 : 30));
}
...
function animate() {
    ...
    if (tutorialPause) { prevTime = time; State.renderer.render(State.scene, State.camera); return; }
    ...
    if (State.gamePhase >= 1) {
        ...
        flashlightDurability = (State.gamePhase === 2) ? 8.0 : 4.0;
    ...
    if (State.flashlightState === 'REPAIRING') {
        ...
        if (!isHoldingFlashlightBtn) {
        ...
        else if (isWalking && State.gamePhase === 0) { 
            ...
            if (repairTimer >= 4.0) { 
                ...
                repairContainer.style.opacity = 0; repairText.style.opacity = 0;
                setTimeout(()=>{ repairContainer.style.display = 'none'; repairText.style.display = 'none'; }, 200);
                flashlightDurability = (State.gamePhase === 2) ? 8.0 : (State.gamePhase === 1 ? 4.0 : 1.0); 
                playPickupSound(); 
            } 
        }
    ...
    if(State.currentLevel < 2) {
        let activeVirtuals = virtualLights.map(vl => ({vl: vl, id: vl.id, dist: State.camera.position.distanceToSquared(new THREE.Vector3(vl.x, vl.y, vl.z))})).filter(v => v.dist < 80000).sort((a, b) => a.dist - b.dist).slice(0, 8);
    ...
    let targetFov = 70; if (isCameraLocked) { targetFov = 25; } if (Math.abs(currentFov - targetFov) > 0.5) { currentFov += (targetFov - currentFov) * delta * 1.5; State.camera.fov = currentFov; State.camera.updateProjectionMatrix(); }
    State.camera.rotation.set(0, 0, 0); State.camera.rotateY(lookState.yaw); State.camera.rotateX(lookState.pitch);
    ...
    } else if (State.gamePhase === 2 && !isJumpscaring) {
    ...
    if(State.currentLevel === 1) { if(State.camera.position.y - State.player.height < waterLevel + 1) { if(!isPlayerInWater) { isPlayerInWater = true; document.getElementById('water-overlay').style.opacity = 1; } State.player.speed = currentBaseSpeed * 0.3; } else { if(isPlayerInWater) { isPlayerInWater = false; document.getElementById('water-overlay').style.opacity = 0; } State.player.speed = currentBaseSpeed; } } else { State.player.speed = currentBaseSpeed; }
    ...
    const rightDir = new THREE.Vector3().crossVectors(camDir, State.camera.up).normalize(); const velocity = new THREE.Vector3(); let effSpeed = isCameraSnapping || isHoldingInteract || isIntroAnim ? 0 : State.player.speed; 
    velocity.addScaledVector(camDir, -finalMoveY * effSpeed * delta); 
    velocity.addScaledVector(rightDir, finalMoveX * effSpeed * delta);
    let nextPos = State.camera.position.clone(); nextPos.x += velocity.x; nextPos.z += velocity.z; velocityY -= 40.0 * delta; nextPos.y += velocityY * delta;
    ...
    if(col.onGround && velocityY <= 0) { 
        ...
        if(finalWalking && !isCameraSnapping && !isHoldingInteract && !isIntroAnim) { 
            bobPhase += delta * 12.0 * (State.player.speed / State.player.baseSpeed); 
    ...
    if(State.camera.position.y < deathYLevel) { triggerJumpscareSequence("FALLEN"); return; }

    const smartBtn = document.getElementById('smart-action-btn');
    nearestItemObj = null; let minDistItem = 9999;
    for (let i = 0; i < State.items.length; i++) { State.items[i].rotation.y += delta; let d = State.camera.position.distanceTo(State.items[i].position); if (d < 12) { if (d < minDistItem) { minDistItem = d; nearestItemObj = State.items[i]; } } }
    let dDoor = exitDoor ? State.camera.position.distanceTo(exitDoor.position) : 9999;
    ...
    if (isHoldingInteract && smartBtnState === 'ITEM') {
        if (finalWalking || !nearestItemObj || isCameraLocked) { 
            ...
            if (interactProgress >= 5.0) { 
                ...
                let lastItemPos = nearestItemObj.position.clone(); let itemIdx = State.items.indexOf(nearestItemObj); 
                if (itemIdx > -1) { 
                    State.scene.remove(nearestItemObj); State.items.splice(itemIdx, 1); itemsCollected++; document.getElementById('item-count').innerText = itemsCollected; 
                    
                    phaseTimer = Math.max(0, phaseTimer - 15.0); 
                    
                    if(State.currentLevel === 0 && itemsCollected <= 3 && State.monsters[itemsCollected - 1] && State.gamePhase === 0) { 
                        let newM = State.monsters[itemsCollected - 1];
                        newM.respawnCooldown = 5.0; 
                        showMessage("Something has awakened in the dark..."); 
                    } 
                    
                    if (itemsCollected === LEVEL_DATA[State.currentLevel].State.items) { createExitDoor(lastItemPos); } 
                } 
            } 
        }
    } else { 
    ...
    if(isBlinking && State.items.length > 0 && whisperGain && whisperOsc) { 
    ...
    State.monsters.forEach(m => {
        ...
        if(dotProd > 0.4 && dist2D < 55) { const raycaster = new THREE.Raycaster(State.camera.position, dirToMonster); const intersects = raycaster.intersectObjects(wallMeshes, true); if(intersects.length === 0 || intersects[0].distance > dist2D) { m.visibleFrame = true; } }
    ...
    if (lockedMonster) {
        let dist2D = State.camera.position.distanceTo(lockedMonster.mesh.position);
        if (dist2D > escapeRadius) { 
            ...
            let targetYaw = Math.atan2(State.camera.position.x - lockedMonster.mesh.position.x, State.camera.position.z - lockedMonster.mesh.position.z);
            ...
            if (State.flashlightState === 'ON' && !isBlinking) { lockedMonster.despawnProgress += 33.33 * delta; } else { lockedMonster.despawnProgress = Math.max(0, lockedMonster.despawnProgress - 20.0 * delta); }
    ...
    let paranoiaIntensity = globalStareTimer / 3.0; vignetteOvl.style.opacity = paranoiaIntensity * 0.7; if(paranoiaIntensity > 0 && !isJumpscaring && !isGlitching && !isPreBlackoutShake) { State.camera.rotation.z += (Math.random() - 0.5) * paranoiaIntensity * 0.1; }
...
function animateMenuParticles() {
    if (document.getElementById('start-screen').style.display !== 'none') {
        mctx.clearRect(0, 0, State.menuCanvas.width, State.menuCanvas.height);
        mctx.shadowBlur = 20;
...
function returnToMenu() {
    State.gameActive = false; if(animationFrameId) cancelAnimationFrame(animationFrameId);
    clearScene();
...
debugBtn.addEventListener('touchstart', (e) => { e.preventDefault(); if(State.gameActive && !isJumpscaring) advanceLevel(); }, {passive: false}); 
debugBtn.addEventListener('click', (e) => { e.preventDefault(); if(State.gameActive && !isJumpscaring) advanceLevel(); });

const nightmareBtn = document.getElementById('debug-nightmare-btn');
nightmareBtn.addEventListener('touchstart', (e) => { e.preventDefault(); if(State.gameActive && State.currentLevel === 0 && phaseTimer > 10.0) phaseTimer = 10.0; }, {passive: false});
nightmareBtn.addEventListener('click', (e) => { e.preventDefault(); if(State.gameActive && State.currentLevel === 0 && phaseTimer > 10.0) phaseTimer = 10.0; });
...
async function startLoadingProcess() {
    ...
    try {
        if(!droneOsc) { droneOsc = State.audioCtx.createOscillator(); droneGain = State.audioCtx.createGain(); droneOsc.type = 'sine'; droneOsc.frequency.value = 50; droneGain.gain.value = 0.2; droneOsc.connect(droneGain); droneGain.connect(State.audioCtx.destination); droneOsc.start(); }
        if(!whisperOsc) { whisperOsc = State.audioCtx.createOscillator(); whisperGain = State.audioCtx.createGain(); whisperOsc.type = 'triangle'; whisperOsc.frequency.value = 800; whisperGain.gain.value = 0; whisperOsc.connect(whisperGain); whisperGain.connect(State.audioCtx.destination); whisperOsc.start(); }
        if(!stareOsc) { stareOsc = State.audioCtx.createOscillator(); stareGain = State.audioCtx.createGain(); stareOsc.type = 'sawtooth'; stareGain.gain.value = 0; stareOsc.connect(stareGain); stareGain.connect(State.audioCtx.destination); stareOsc.start(); }
        if(!proxOsc) { proxOsc = State.audioCtx.createOscillator(); proxGain = State.audioCtx.createGain(); proxOsc.type = 'sawtooth'; proxOsc.frequency.value = 100; proxGain.gain.value = 0; proxOsc.connect(proxGain); proxGain.connect(State.audioCtx.destination); proxOsc.start(); }
        ...
        if(!nightmareNoiseSrc) {
            ...
            nightmareNoiseGain = State.audioCtx.createGain(); nightmareNoiseGain.gain.value = 0;
            nightmareNoiseSrc = State.audioCtx.createBufferSource(); nightmareNoiseSrc.buffer = noiseBuffer; nightmareNoiseSrc.loop = true;
            let noiseFilter = State.audioCtx.createBiquadFilter(); noiseFilter.type = 'lowpass'; noiseFilter.frequency.value = 800;
            nightmareNoiseSrc.connect(noiseFilter); noiseFilter.connect(nightmareNoiseGain); nightmareNoiseGain.connect(State.audioCtx.destination);
    ...
    setTimeout(() => {
        ...
        currentFov = 70; State.camera.fov = currentFov; State.camera.updateProjectionMatrix();
        
        State.flashlightState = 'ON'; repairTimer = 0; isHoldingFlashlightBtn = false;
        flashlightDurability = 1.0; 
        normalTick = 0; sabotageTick = 0; nightmareTurnOffTick = 0;
        tutorialPause = false;
        
        State.gamePhase = 0; phaseTimer = 300.0; slaughterSpeedMultiplier = 1.0;
        ...
        State.gameActive = true; prevTime = performance.now();
